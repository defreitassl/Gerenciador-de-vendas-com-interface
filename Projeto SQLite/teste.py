...
def janela_editar_produto():
    layout_editar = [
        ...
        [sg.Input(key='atributo', size=(15, 1))],
    ...
    return sg.Window('Editar Produtos', layout_editar, finalize=True)
...
tab_cadastro_produtos = [
    ...
    [sg.Combo(nome_produtos, visible=False, default_value='Produtos', key='escolher_produtos', size=(15, 1)), sg.Button("Visualizar", key='escolher', visible=False), sg.Button('Deletar', key='deletar_produto', button_color='red', visible=False), sg.Button('Editar', key='editar_produto', visible=False)],
...
tab_cadastro_vendas = [
    [sg.Text("Produto:", size=(12, 1)), sg.Combo(values=[], default_value="Produtos", key='vender_produtos', size=(15, 1), enable_events=True)],
...
janela = sg.Window("Gerenciamento de produtos", layout, element_justification='center', font=('Helvetica', 16), element_padding=10, size=(1000, 700), finalize=True)
janela_editar = None
...
while True:
    ...
    if window == janela and event in (sg.WIN_CLOSED, 'cancelar'):
        ...
        for item in produtos_cadastrados:
            if item.nome == produto:
                nome, dsc, qnt, prc = item.nome, item.descricao, item.quantidade_estoque, item.preco
    ...
    elif window == janela and event == 'limpar_vendas':
        ...
        janela['tabela_relatorio'].update(values=produtos_relatorio)
